...
6. **Ask before saving — two separate prompts.** Saving locally and uploading to OneDrive are two independent decisions; ask about them one at a time with the AskUserQuestion tool, in this order.
   1. **Ask to save locally.** Once the voice-checked draft is ready, ask the user whether they want to save the article locally.
      - If **yes**, write the article as a self-contained HTML file in a `galina/generated galina articles/` folder **inside the user's current working directory** (create the folder if it doesn't exist), named after a slugified title (e.g., `the-board-s-new-mandate-ai-governance.html`). Use the template below, wrapping each stat in a real source link. **Do not write into `${CLAUDE_SKILL_DIR}`** — that is the (potentially read-only, update-managed) install location; the blank `generated galina articles/` folder shipped there is only a structural template.
      - If **no**, do not write the file — present the article inline (or follow whatever the user prefers).
   2. **Then ask, separately, to upload to OneDrive via Composio.** After handling the local-save answer, ask the user whether they also want to upload the article to OneDrive. Ask this regardless of the local-save answer (if they declined the local save, write the HTML to a temporary file first so there is something to upload).
      - If **yes**, upload the HTML file to a folder named **`Claude Generated Articles`** in the user's OneDrive (auto-created if it doesn't exist):
        - **Ensure a connection first.** If there is no active `one_drive` connection (a prior `COMPOSIO_SEARCH_TOOLS` result will report `has_active_connection: false`, or the upload returns a no-connection error), call `COMPOSIO_MANAGE_CONNECTIONS` with toolkit `one_drive` (action `add`), present the returned `redirect_url` to the user as a markdown link, then poll with `COMPOSIO_WAIT_FOR_CONNECTIONS` until the connection is Active before uploading.
        - **Upload.** Call `ONE_DRIVE_ONEDRIVE_UPLOAD_FILE` with:
          - `file`: the HTML file — `{ name: "<slug>.html", mimetype: "text/html" }` plus the local file path (the `file` field is file-uploadable, so the MCP layer reads the local path directly).
          - `folder`: `"/Claude Generated Articles"` (auto-created if absent).
          - `conflict_behavior`: `"rename"` (the default — this preserves earlier uploads rather than overwriting them; the response `name`/`webUrl` reflects the final stored filename).
        - **An upload failure must not undo the local save.** If a local file was saved in step 1, it is the source of truth — if the OneDrive upload fails (e.g., the user declines to connect, or auth times out), keep that local file and report the upload failure rather than aborting or deleting anything.
      - If **no**, skip the upload.
7. **Report to the user**: the local file path (if saved), the **OneDrive destination** (the `Claude Generated Articles` folder and the uploaded filename, or the upload failure / "not uploaded" if applicable), the title, the word count / read-time, and the list of statistics used with their sources. Confirm that every stat is backed by a real citation (there should be no illustrative or unverified numbers).
