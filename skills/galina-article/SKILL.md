...
6. **Ask before saving.** Once the voice-checked draft is ready, use the AskUserQuestion tool to ask the user whether they want to save the article. Saving is a single combined action: the local HTML file **and** the OneDrive upload happen together — one "yes" covers both.
   - If **yes**, do both of the following:
     1. **Save locally.** Write the article as a self-contained HTML file in a `galina/generated galina articles/` folder **inside the user's current working directory** (create the folder if it doesn't exist), named after a slugified title (e.g., `the-board-s-new-mandate-ai-governance.html`). Use the template below, wrapping each stat in a real source link. **Do not write into `${CLAUDE_SKILL_DIR}`** — that is the (potentially read-only, update-managed) install location; the blank `generated galina articles/` folder shipped there is only a structural template.
     2. **Upload the same file to OneDrive via Composio.** Upload it to a folder named **`Claude Generated Articles`** in the user's OneDrive (the folder is auto-created if it doesn't exist).
        ...
        - **Upload.** Call `ONE_DRIVE_ONEDRIVE_UPLOAD_FILE` with:
          - `file`: the local HTML file just written — `{ name: "<slug>.html", mimetype: "text/html" }` plus the local file path (the `file` field is file-uploadable, so the MCP layer reads the local path directly).
        ...
        - **Don't let an upload failure lose the local save.** The local file is the source of truth. If the OneDrive upload fails (e.g., the user declines to connect, or auth times out), keep the successfully-saved local file and report the upload failure to the user rather than aborting or deleting anything.
   - If **no**, do neither — present the article inline (or follow whatever the user prefers) instead.
7. **Report to the user**: the local file path (if saved), the **OneDrive destination** (the `Claude Generated Articles` folder and the uploaded filename, or the upload failure if it did not succeed), the title, the word count / read-time, and the list of statistics used with their sources. Confirm that every stat is backed by a real citation (there should be no illustrative or unverified numbers).
