...
6. **Ask before saving.** Once the voice-checked draft is ready, use the AskUserQuestion tool to ask the user whether they want to save the article.
   - If **yes**, save the article as a self-contained HTML file in a `galina/generated galina articles/` folder **inside the user's current working directory** (create the folder if it doesn't exist), named after a slugified title (e.g., `the-board-s-new-mandate-ai-governance.html`). Use the template below, wrapping each stat in a real source link. **Do not write into `${CLAUDE_SKILL_DIR}`** — that is the (potentially read-only, update-managed) install location; the blank `generated galina articles/` folder shipped there is only a structural template.
